...
public function __construct(){
	  ...
	  public function index()
	   {
		   ...
		   $counta=$this->admin->count_role("SELECT * FROM `roles`,user_roles WHERE user_roles.role_id=roles.role_id AND user_roles.user_id='5' AND user_roles.role_id='2'");
			...
			{
		   $a=1;
		  $data=$this->admin->get_specific_data('status',$a,'roles');
		
		$this->load->view('role',compact('data'));
		extract($_POST);
		if(isset($submit))
		{
			if(empty($_POST['flag']))
	...
	$login=$this->session->userdata('login');
	 $uid=$this->admin->get_x('user_id','email',$login,'users');
			...
			$get1=$this->admin->insert_data('roles',$data);
	   ...
	   {
		$id=$_GET['id'];
		$data=$this->admin->get_specific_data('role_id',$id,'roles');
		$this->load->view('role_edit',compact('data'));
		extract($_POST);
		if(isset($submit))
		{
			if(empty($_POST['flag']))
	...
	$login=$this->session->userdata('login');
	 $uid=$this->admin->get_x('user_id','email',$login,'users');
			...
			$get1=$this->admin->edit_data('role_id',$id,$data,'roles');
	  ...
	  {
		  $id=$_GET['id'];
		  $user=$this->admin->delete_data('role_id',$id,'roles');
